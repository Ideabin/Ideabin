...
def create_idea():
    ...
    if current_user.is_anonymous():
        ...
        text("INSERT INTO idea values(:id, :t, :con, :desc, :uid, :tags)") \
            .params(id=uuid.UUID(), t=title, con=dt.datetime.utcnow(),
                    desc=desc, uid=User.get_anon().user_id, tags=tags)
    else:
        new = Idea.new(title, desc, current_user.user_id, tags)
        text("INSERT INTO idea values(:id, :t, :con, :desc, :uid, :tags)") \
            .params(id=uuid.UUID(), t=title, con=dt.datetime.utcnow(),
                    desc=desc, uid=current_user.user_id, tags=tags)
